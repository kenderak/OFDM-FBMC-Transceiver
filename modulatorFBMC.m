...
% Normal symbols
SymbolsFOv(S,NrOfSymbols) = eps*1i;
...
% OFFSETSYMBOLS
SymbolsFOvOff(S,NrOfSymbols)=eps*1i;
...
SymbolsFSpreadFilt(S,NrOfSymbols) = eps*i;
SymbolsFSpreadOffFilt(S,NrOfSymbols) = eps*i;
for i=1:NrOfSymbols
...
Scale = 1;
% Oversampling
SymbolsTOv = zeros(S * OV,NrOfSymbols);
...
SymbolsTOffOv = zeros(S*OV,NrOfSymbols);
...
signalTx = zeros((N/2 + N * NrOfSymbols + (K-1)*N)*OV,1);
for i=1:NrOfSymbols
